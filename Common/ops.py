...
def up_block(inputs, up_ratio, scope='up_block', is_training=True, bn_decay=None):
    with tf.variable_scope(scope,reuse=tf.AUTO_REUSE):
        ...
        """ Feature up sampling by conv + PointShuffler
            revised by Zezeng li in 3 Mar 2021
        ## !!! the output channel should be checked !!!
        """  
        """
        net = conv2d(net, out_dim, [1, 1],
        ...
        net = PointShuffler(net, up_ratio) 
                            
        #print("################ Have Done ! ################")                    
        # Feature replication in original version
...
def down_block_deep(inputs,up_ratio,scope='down_block_deep',is_training=True,bn_decay=None):
    with tf.variable_scope(scope,reuse=tf.AUTO_REUSE):
        net = inputs
        net = tf.reshape(net,[tf.shape(net)[0],up_ratio,-1,tf.shape(net)[-1]])
        net = tf.transpose(net, [0, 2, 1, 3])
...
def feature_extraction(inputs, scope='feature_extraction2', is_training=True, bn_decay=None, up_feature=True):
    ...
    with tf.variable_scope(scope,reuse=tf.AUTO_REUSE):
        ...
        l1_features, l1_idx = dense_conv(l0_features, growth_rate=growth_rate, n=dense_n, k=knn,
                                                  ...
                                                  bn_decay=bn_decay)
        l1_features = tf.concat([l1_features, l0_features], axis=-1)  # (24*4)+24=120
        ...
        l2_features = tf.concat([l2_features, l1_features], axis=-1)  # 120+120=240
        '''
        print('!!!'+'**'*20+'!!!')
        sess = tf.Session()
        print('l0_features.shape=',sess.run(tf.shape(l0_features)))
        print('l1_features.shape=',sess.run(tf.shape(l1_features)))
        print('l2_features.shape=',sess.run(tf.shape(l2_features)))'''
        ...
        if up_feature:
            ...
            l3_features, l3_idx = dense_conv(l3_features, growth_rate=growth_rate, n=dense_n, k=knn,
                                                  scope="layer3", is_training=is_training, bn=use_bn, bn_decay=bn_decay)
            l3_features = tf.concat([l3_features, l2_features], axis=-1)  # 240+120=360
            l3_features = tf.expand_dims(l3_features, axis=2)
            #print('l3_features.shape=',sess.run(tf.shape(l3_features)))
            return l3_features
            '''
            l2_features = tf.expand_dims(l2_features, axis=2)
            return l2_features
...
## revised by Zezeng li in 8 Mar 2021
## point down_projection_unit
def down_projection_unit(inputs,down_ratio,scope="down_projection_unit",is_training=True,bn_decay=None):
    with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
        H = conv2d(inputs, 128, [1, 1],
                               padding='VALID', stride=[1, 1],
                               bn=False, is_training=is_training,
                               scope='conv0', bn_decay=bn_decay)
        '''
