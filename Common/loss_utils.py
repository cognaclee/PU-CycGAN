...
def pc_distance(pcd1,pcd2,dis_type='emd',radius=1):
    if dis_type == 'cd':
        ...
        return chamfer(pcd1,pcd2,radius=radius)+hausdorff(pcd1,pcd2,radius=radius)
    else:
...
def hausdorff(pcd1, pcd2, radius=1.0):
    dists_forward, _, dists_backward, _ = tf_nndistance.nn_distance(pcd1, pcd2)
    hd_value = tf.reduce_max([tf.reduce_max(dists_forward, reduction_indices=[1]),tf.reduce_max(dists_backward, reduction_indices=[1])],reduction_indices=[0])
    hd_value = hd_value / radius
    hd_loss = tf.reduce_mean(hd_value)
    return hd_loss
