...
def wgan_QCloss(pred_d_real, pred_d_fake,HStar_real,HStar_fake):
    ...
    #sess = tf.Session()
    #print('Dloss=',sess.run(loss))
    return loss

'''
input:
    preds: Output of discriminator at fake_points
    real_points: real points or source domain points
    fake_points: fake points or target domain points
    KCoef: Lipschitz condition of order KCoef
    gamma: hyperparameters of regular term 
output:  
    gradient_penalty: WGAN_QC regulation
''' 
"""
def gradient_penalty_WQC(fake_preds, fake_points, KCoef, pc_dist):
    gradients = tf.gradients(fake_preds, fake_points, gate_gradients=True)[0]
    #sess = tf.Session()
    #print('gradients.shape=',sess.run(tf.shape(gradients)))
    gradient_norm = tf.norm(tf.norm(gradients,ord=2,axis=2),ord=2,axis=1)
    gradient_penalty = tf.reduce_mean(tf.square(gradient_norm - KCoef*pc_dist))
    #gradient_penalty = tf.reduce_mean(tf.square(gradient_norm - 1))
    #print('gradient_penalty=',sess.run(tf.shape(gradient_penalty)))
...
def gradient_penalty_WQC(discriminator, fake_points, KCoef, pc_dist):
    ...
    #print('gradients.shape=HAHAHAHA')
    return gradient_penalty
