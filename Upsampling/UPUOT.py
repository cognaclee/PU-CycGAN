...
class Model(object):
  ...
  def allocate_placeholders(self):
      ...
      self.input_y = tf.placeholder(tf.float32, shape=[self.opts.batch_size, int(self.up_ratio*self.opts.num_point),3])
  ...
  def build_model(self):
      ...
      self.distance_midx = self.opts.midDist_w * pc_distance(self.up_x, self.input_x, dis_type='cd', radius=self.pc_radius)
      self.distance_midy = self.opts.midDist_w * pc_distance(self.down_y, self.input_y, dis_type='cd', radius=self.pc_radius)
      ...
      if self.gan_type == 'lsgan':
          ...
          self.total_loss_x = self.G_loss_x + self.pu_loss_x
          self.regular_x = self.opts.WQC_gamma*gradient_penalty_WQC(self.DS,self.down_x,self.opts.WQC_KCoef, self.distance_x)
          self.D_loss_x = wgan_QCloss(self.pred_real_x, self.pred_fake_x,self.HStar_real_x,self.HStar_fake_x)+ self.regular_x
          ...
          self.pu_loss_y = self.opts.fidelity_w * self.distance_y + self.uniform_loss_y + self.repulsion_loss_y+ self.distance_midy
          self.total_loss_y = self.G_loss_y + self.pu_loss_y
          self.regular_y = self.opts.WQC_gamma*gradient_penalty_WQC(self.DD,self.up_y, self.opts.WQC_KCoef, self.distance_y)
          self.D_loss_y = wgan_QCloss(self.pred_real_y, self.pred_fake_y,self.HStar_real_y,self.HStar_fake_y)+ self.regular_y
  ...
  def setup_optimizer(self):
      ...
      gen_update_ops = [op for op in tf.get_collection(tf.GraphKeys.UPDATE_OPS) if op.name.startswith("generator")] + [op for op in tf.get_collection(tf.GraphKeys.UPDATE_OPS) if op.name.startswith("degenerater")]
      
      gen_tvars = [var for var in tf.trainable_variables() if var.name.startswith("generator")] + [var for var in tf.trainable_variables() if var.name.startswith("degenerater")]

      with tf.control_dependencies(gen_update_ops):
          self.G_optimizers = tf.train.AdamOptimizer(learning_rate_g, beta1=self.opts.beta).minimize(self.total_G_loss,var_list=gen_tvars,colocate_gradients_with_ops=True,global_step=self.global_step,name='Adam_G')
      ...
      self.D_optimizers = tf.train.AdamOptimizer(learning_rate_d, beta1=self.opts.beta).minimize(self.total_D_loss,\
                                                  self.global_step,var_list=dis_tvars,name='Adam_D')
  ...
  def train(self):
      ...
      for epoch in range(restore_epoch, self.opts.training_epoch):
          ...
          for batch_idx in range(fetchworker.num_batches):
              ...
              _,total_D_loss,d_summary = self.sess.run([self.D_optimizers,self.total_D_loss,self.d_summary_op],feed_dict=feed_dict)
              ...
              feed_dict = {self.input_x: batch_input_x,
                           ...
                           self.pc_radius: batch_radius,             
                           self.is_training: True}
              _, total_G_loss,summary = self.sess.run([self.G_optimizers, self.total_G_loss, self.g_summary_op], feed_dict=feed_dict)
  ...
  def pc_prediction(self, pc):
      ...
      ## FPS sampling for synthesis data, polar sampling for kitti
      if self.opts.which_model == 'real':
          seed = polarSampling(points.eval(),seed1_num)
      else:
          seed = farthest_point_sample(seed1_num, points).eval()[0]
      
      seed_list = seed[:seed1_num]
      print("sampling cost is ", time() - start)
  ...
  def down_prediction(self, pc):
      ## get patch seed from farthestsampling
      points = tf.convert_to_tensor(np.expand_dims(pc,axis=0),dtype=tf.float32)
      start= time()
      print('------------------patch_num_point:',self.opts.patch_num_point)
      seed1_num = int(pc.shape[0] / (self.up_ratio*self.opts.patch_num_point) * self.opts.patch_num_ratio)
      ...
      patches = pc_util.extract_knn_patch(pc[np.asarray(seed_list), :], pc, int(self.up_ratio*self.opts.patch_num_point))

      for point in tqdm(patches, total=len(patches)):
          down_point = self.patch_prediction(point)
          down_point = np.squeeze(down_point,axis=0)
          input_list.append(point)
          down_point_list.append(down_point)

      return input_list, down_point_list
      
  def testSparserWithObject(self):
      print("Downsampling test start!")
      self.inputs = tf.placeholder(tf.float32, shape=[1, int(self.up_ratio*self.opts.patch_num_point), 3])
      is_training = tf.placeholder_with_default(False, shape=[], name='is_training')
      Deg = Degenerater(self.opts, is_training, name='degenerater')
      self.pred_pc = Deg(self.inputs)
      ...
      for point_path in samples:
          ...
          input_list, pred_list = self.down_prediction(pc)
          ...
          pred_pc = np.reshape(pred_pc,[-1,3])
          path = os.path.join(self.opts.out_folder, point_path.split('/')[-1][:-4])
          idx = farthest_point_sample(out_point_num, pred_pc[np.newaxis, ...]).eval()[0]
          pred_pc = pred_pc[idx, 0:3]
          np.savetxt(path + '.xyz',pred_pc,fmt='%.6f')
          
  def testSparserWithPatch(self,h5_file_name):
      self.inputs = tf.placeholder(tf.float32, shape=[self.opts.batch_size, int(self.up_ratio*self.opts.patch_num_point), 3])
      is_training = tf.placeholder_with_default(False, shape=[], name='is_training')
      Deg = Degenerater(self.opts, is_training, name='degenerater')
      self.pred_pc = Deg(self.inputs)
      ...
      input_x = np.zeros((100*self.opts.batch_size,self.opts.patch_num_point,3))
      input_y = np.zeros((100*self.opts.batch_size,int(self.up_ratio*self.opts.patch_num_point),3))
      down_y = np.zeros((100*self.opts.batch_size,self.opts.patch_num_point,3))
      fetchworker = Fetcher(self.opts)
      fetchworker.start()
      for idx in range(100):
          batch_input_x, batch_input_y,batch_radius = fetchworker.fetch()
          pred = self.sess.run([self.pred_pc], feed_dict={self.inputs: batch_input_y})
          down_y_val = np.squeeze(pred)
          input_x[idx*self.opts.batch_size:(idx+1)*self.opts.batch_size,:,:]=batch_input_x
          input_y[idx*self.opts.batch_size:(idx+1)*self.opts.batch_size,:,:]=batch_input_y
          down_y[idx*self.opts.batch_size:(idx+1)*self.opts.batch_size,:,:]=down_y_val
...
class FarthestSampler:
    ...
    def __call__(self, pts, k):
        farthest_idx = np.zeros(k, dtype=np.int64)
        farthest_idx[0] =  np.random.randint(len(pts))
        distances = self._calc_distances(pts[farthest_idx[0]], pts)
        for i in range(1, k):
            farthest_idx[i] =  np.argmax(distances)
            distances = np.minimum(
                distances, self._calc_distances(pts[farthest_idx[i]], pts))
        return farthest_idx

# ----------------------------------------
# polar Sampling on kitti
# ----------------------------------------
def polarSampling(point,seed_num):
    pc = np.squeeze(point)

    nums = np.shape(pc)[0]
    probi=np.zeros(nums)
    for i in range(nums):
        probi[i] = 1/math.sqrt(pc[i,0]**2+pc[i,1]**2+0.01)
    
    probi = 0.1*nums*probi/sum(probi)
    
    unif = np.random.uniform(low=0,high=1,size=nums)
    index = np.where(unif<probi)
    indLen = np.shape(index)[1]
    if indLen<seed_num:
        seed = np.zeros(seed_num,np.int64)
        seed[:indLen]=np.squeeze(np.array(index))
        need = seed_num-indLen
        seed[indLen:]=np.random.randint(0,nums,need)
    else:
        ind = np.squeeze(np.array(index))
        choose = pc[ind,:]
        far = FarthestSampler()
        seed = ind[far(choose,seed_num)]
    return seed