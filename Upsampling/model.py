...
class Model(object):
  def __init__(self,opts,sess,gan_type = 'qcgan'):
      self.sess = sess
      self.opts = opts
      self.opts.WQC_gamma = 0.05
      self.gan_type = gan_type
      #self.opts.epoch_per_save = 1
      #self.opts.steps_per_visu = 20
      #self.opts.visulize = True
  ...
  def allocate_placeholders(self):    
      ...
      self.pc_difference = tf.placeholder(tf.float32,shape=[], name='pc_difference')
      self.HStar_real = tf.placeholder(tf.float32, shape=[self.opts.batch_size], name='HStar_real')
      self.HStar_fake = tf.placeholder(tf.float32, shape=[self.opts.batch_size], name='HStar_fake')
  ...
  def build_model(self):
      ...
      self.dis_loss = pc_distance(self.G_y, self.input_y, radius=self.pc_radius)
      ...
      if self.gan_type == 'lsgan':
          ...
          self.pu_loss = self.opts.fidelity_w * self.dis_loss + self.uniform_loss + self.repulsion_loss + tf.losses.get_regularization_loss()
          ...
          pred_d_real = self.D(self.input_y)
          pred_d_fake = self.D(self.G_y)
          
          #self.G_gan_loss = self.opts.gan_w*tf.reduce_mean(tf.square(pred_d_fake - 1))
          self.G_gan_loss = self.opts.gan_w*tf.reduce_mean(tf.square(pred_d_fake - pred_d_real))
          self.pu_loss = self.opts.fidelity_w * self.dis_loss + self.uniform_loss +  self.repulsion_loss + tf.losses.get_regularization_loss()
          self.total_gen_loss = self.G_gan_loss + self.pu_loss
          
          #self.regular = self.opts.WQC_gamma*gradient_penalty_WQC(pred_d_fake, self.G_y, self.opts.WQC_KCoef, self.pc_difference)
          #self.regular = self.opts.WQC_gamma*gradient_penalty_WGP(self.D, self.G_y, self.input_y)
          self.regular = self.opts.WQC_gamma*gradient_penalty_WQC(self.D, self.G_y, self.opts.WQC_KCoef, self.pc_difference)
          self.D_loss = wgan_QCloss(pred_d_real, pred_d_fake,self.HStar_real,self.HStar_fake)+ self.regular
          #self.D_loss = discriminator_loss(self.D,self.input_y,self.G_y)
  ...
  def setup_optimizer(self):
      ...
      with tf.control_dependencies(gen_update_ops):
          self.G_optimizers = tf.train.AdamOptimizer(learning_rate_g, beta1=self.opts.beta).minimize(self.total_gen_loss,var_list=gen_tvars,colocate_gradients_with_ops=True,global_step=self.global_step)
  ...
  def train(self):
      ...
      Gene_y = np.zeros([self.opts.batch_size, int(4*self.opts.num_point),3],dtype=np.float)
      dis_loss = 1.0
      ...
      for epoch in range(restore_epoch, self.opts.training_epoch):
          ...
          for batch_idx in range(fetchworker.num_batches):
              ...
              # Update D network
              HStar_fake, HStar_real = H_Star_Solution(Gene_y, batch_input_y, self.opts.WQC_KCoef)
              feed_dict = {self.input_x: batch_input_x,
                           self.input_y: batch_input_y,
                           self.pc_radius: batch_radius,
                           self.HStar_fake: HStar_fake,
                           self.HStar_real: HStar_real,
                           self.pc_difference: dis_loss,
              ...
              for i in range(self.opts.gen_update):
                  ...
                  _, g_total_loss, summary, Gene_y,dis_loss = self.sess.run([self.G_optimizers, self.total_gen_loss, self.g_summary_op,self.G_y,self.dis_loss], feed_dict=feed_dict)
